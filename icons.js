/* ════════════════════════════════════════════════════════════════
   Bastion — inline SVG icon system for the marketing site.
  ...
  function hydrate(root) {
    (root || document).querySelectorAll('i[class*="ph-"]').forEach(function (el) {
      ...
      if (markup) { el.innerHTML = markup; el.dataset.icDone = '1'; el.dataset.icName = name; }
        ...
        el.dataset.icDone = '1';
        if (window.console) console.warn('[bastion icons] missing icon:', name);
      }
    });
  }

  window.BastionIcons = { hydrate: hydrate, svg: svg, has: function (n) { return !!(I[ALIAS[n] || n]); } };
